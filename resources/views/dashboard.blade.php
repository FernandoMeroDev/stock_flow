<x-layouts.app :title="__('Dashboard')">
    <div class="flex flex-wrap justify-center sm:justify-evenly">
        ...
        <x-dashboard.card :href="route('products.index')" permissions="products">
            <x-svg.box class="w-32 h-32" />
            <p class="text-center">Productos</p>
        </x-dashboard.card>
        <x-dashboard.card :href="route('sales.index')" permissions="sales">
            <x-svg.report-linechart class="w-32 h-32" />
            <p class="text-center">Ventas</p>
        </x-dashboard.card>
        <x-dashboard.card :href="route('inventories.index')" permissions="inventories">
            <x-svg.report-text class="w-32 h-32" />
            <p class="text-center">Inventarios</p>
        </x-dashboard.card>
        <x-dashboard.card :href="route('warehouses.index')" permissions="warehouses">
